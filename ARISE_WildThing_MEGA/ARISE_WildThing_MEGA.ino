...
*/
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////
      // set motor directions (Note: dir1_opp and dir2_opp not used for SPARK but needed to keep code consistent)
      if (motorLVel_Filt >= 0) {
        motorLVel_Filt = motorLVel_Filt + min( maxAccel, max( -maxDecel, motorLVel - motorLVel_Filt ) );
      } else {
        motorLVel_Filt = motorLVel_Filt + max( -maxAccel, min( maxDecel, motorLVel - motorLVel_Filt ) );
      ...
      if (motorRVel_Filt >= 0) {
        motorRVel_Filt = motorRVel_Filt + min( maxAccel, max( -maxDecel, motorRVel - motorRVel_Filt ) );
      } else {
        motorRVel_Filt = motorRVel_Filt + max( -maxAccel, min( maxDecel, motorRVel - motorRVel_Filt ) );
     }
     if (motorDriverType < 3) {
         if (motorRVel_Filt >= 0) {
            digitalWrite(dir2, HIGH);
            digitalWrite(dir2_opp, LOW);
          } else {
        ...
        // no need to set direction pins for vnh5019
      }
